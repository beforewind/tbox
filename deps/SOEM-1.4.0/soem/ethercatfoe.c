...
// from guowei
int ecx_FOEwrite_file(ecx_contextt *context, uint16 slave, const char *filename, uint32 password, int psize, unsigned char *p, int timeout)
{
   ec_FOEt *FOEp, *aFOEp;
   int wkc;
   int32 packetnumber, sendpacket = 0;
   ...
   if (wkc > 0) /* succeeded to place mailbox in slave ? */
   {
      do
      { if(psize <= 1000)
            int xin = 1;//{std::cout<<"the last packet is coming"<< std::endl;}
         worktodo = FALSE;
         /* clean mailboxbuffer */
         ec_clearmbx(&MbxIn);
         /* read slave response */
...
// from guowei
int ec_FOEwrite_file(uint16 slave, const char *filename, uint32 password, int psize, unsigned char *p, int timeout)
{
   return ecx_FOEwrite_file(&ecx_context, slave, filename, password, psize, p, timeout);
}
