...
{
public:
    const uint32_t MAX_QUEUE_SIZE = 1;
    ...
    int32_t setIdCallback(
        const std::function<void(int32_t)> &requestIdCallback,
        const std::function<void(int32_t)> &responseIdCallback);
    int32_t canAccept();
    int32_t sendRequest(Request &req);
    int32_t recvResponse(Response &rsp);
    int32_t dropResponse();
    std::map<uint16_t, std::vector<uint8_t>> getDinStatus();
    ...
    bool pause();
    bool resume();
    bool release(std::vector<uint16_t>& deviceIdList);
    bool stop();
    bool isRunning();
    // int32_t txQueueSize();
...
private:
    ...
    Request currReq = {};
    Response currRsp = {};
    std::map<uint16_t, std::vector<uint8_t>> currDinStatus;

    std::function<void(Request)> req_cb = nullptr;
    std::function<void(Response)> rsp_cb = nullptr;
    std::function<void(int32_t)> reqId_cb = nullptr;
    std::function<void(int32_t)> rspId_cb = nullptr;
