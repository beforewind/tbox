...
bool EctBus::waitUntilAllSlavesReachedOP()
{
    ...
    if (ec_config_mul(&ecx_context_ins, FALSE, &m_IOmap) > 0)
    {
        ...
        // while (*(ecx_context_ins.ecaterror))  // TODO: 去掉死循环  //
        if (*(ecx_context_ins.ecaterror))
        {
            // printf("%s", ecx_elist2string(&ecx_context_ins));
            TERROR("Error in EtherCAT configuration {}", ecx_elist2string(&ecx_context_ins));
            return false;
        }
        // printf("%d slaves found and configured.\n", *(ecx_context_ins.slavecount));
        TDEBUG("{} slaves found and configured.", *(ecx_context_ins.slavecount));
        ...
        if (ecx_context_ins.slavelist[0].state != EC_STATE_SAFE_OP)
        {
            ...
            TERROR("Not all slaves reached safe operational state.");
            ...
            for (int i = 1; i <= *(ecx_context_ins.slavecount); i++)
            {
                if (ecx_context_ins.slavelist[i].state != EC_STATE_SAFE_OP)
                {
                    ...
                    //     i, ecx_context_ins.slavelist[i].state, ecx_context_ins.slavelist[i].ALstatuscode,
                    //     ec_ALstatuscode2string(ecx_context_ins.slavelist[i].ALstatuscode));
                    TERROR("Slave {} State={} StatusCode={} : {}",
                           i, ecx_context_ins.slavelist[i].state, ecx_context_ins.slavelist[i].ALstatuscode,
                           ec_ALstatuscode2string(ecx_context_ins.slavelist[i].ALstatuscode));
                }
            }
        ...
        else
        {
            ...
            for (int i = 1; i <= *(ecx_context_ins.slavecount); i++)
            {
                if (ecx_context_ins.slavelist[i].state != EC_STATE_OPERATIONAL)
                {
                    ...
                    //    i, ecx_context_ins.slavelist[i].state, ecx_context_ins.slavelist[i].ALstatuscode,
                    //    ec_ALstatuscode2string(ecx_context_ins.slavelist[i].ALstatuscode));
                    TDEBUG("Slave {} State={} StatusCode={} : {}",
                           i, ecx_context_ins.slavelist[i].state, ecx_context_ins.slavelist[i].ALstatuscode,
                           ec_ALstatuscode2string(ecx_context_ins.slavelist[i].ALstatuscode));
                    nonOpStateCount++;
                }
            }
            if (nonOpStateCount > 0)
            {
                TERROR("Not all slaves reached OP");
                return false;
            }
            else
            {
                m_busReady = true;
                TDEBUG("All slaves reached OP");
            }
        }
        return true;
    }
...
int EctBus::updateBus()
{
    ...
    if (m_busReady)
    {
        // updateWriteStamp_ = std::chrono::high_resolution_clock::now();
        {
            ...
            rt = ecx_send_processdata_group(&ecx_context_ins, 0);
        }
        if (rt <= 0)
        {
            TERROR("ecx_send_processdata_group fail"); // >0 transmited
            return -1;
        }
        // updateReadStamp_ = std::chrono::high_resolution_clock::now();
        {
            std::lock_guard<std::recursive_mutex> guard(contextMutex_);
            lastWorkCounter = ecx_receive_processdata_group(&ecx_context_ins, 0, EC_TIMEOUTMON * 10);
        }
        // updateReadStamp_ = std::chrono::high_resolution_clock::now();
        //  TDEBUG("time used {} microseconds", std::chrono::duration_cast<std::chrono::microseconds>(updateReadStamp_ - updateWriteStamp_).count());
        //  m_expectedWKC is a fixed value, decided by the number of devices.
        if (lastWorkCounter >= m_expectedWKC)
        { // no need to update. TODO: 这里是不是要更新m_expectedWKC？？ //
            return 0;
        }
        else
        {
            {
                std::lock_guard<std::recursive_mutex> guard(contextMutex_);
                std::cout << "For all slaves alStatusCode: "
                          << std::hex << ecx_context_ins.slavelist[0].ALstatuscode << " "
                          << ec_ALstatuscode2string(ecx_context_ins.slavelist[0].ALstatuscode) << std::endl;
            }
            TERROR("Workcounter {} is less than expected {}", lastWorkCounter, m_expectedWKC);
            return -1;
        }
        //       return true;//pigfly
    }
...
int EctBus::createSlaves()
{
    ...
    if (slavesCreated)
    {
        ...
        TERROR("Slave have already been created! There must be a logic error.");
        return -1;
    }
...
bool EctBus::getDinStatus(std::map<uint16_t, std::vector<uint8_t>> &rspDin)
{
    rspDin.clear();
...
bool EctBus::setDoutControl(std::map<uint16_t, std::vector<uint8_t>> &reqDout)
{
    for (auto dout : reqDout)
    {
        if (m_slaveMap.count(dout.first) > 0) // TODO:
...
// 设置单个bit的dout/control的状态 //
// 在run活跃期间有效 //
bool EctBus::setDoutControlSingle(uint16_t deviceId, uint8_t doutIndex, bool doutValue)
{
    if (m_slaveMap.count(deviceId) > 0) // device not exists
    {
        // 读取该PDO在PC buffer中的缓存值，然后修改对应bit，然后写入缓存，等待在run线程中更新到device //
        std::vector<uint8_t> currDoutControl = m_slaveMap[deviceId]->getPDO(INDX_STAT_PDO, 2);
        uint8_t currByte = currDoutControl.at(doutIndex/8);
        uint8_t newByte = currByte & (~(doutValue << (doutIndex % 8)));
        currDoutControl.at(doutIndex/8) = newByte;
        m_slaveMap[deviceId]->setPDO(INDX_STAT_PDO, currDoutControl);
    }
    return true;
}
            ...
            if (ODlist.Index[i] >= 0x6000 && ODlist.Index[i] < 0xA000)
            {
                ...
                for (uint16_t j = 0; j < (ODlist.MaxSub[i] + 1); j++) // ODlist.MaxSub[i]是指最大sub index的值，所以要加1控制循环边界 //
                {
                    ...
                    if (isSubIndex000Entry)
                    {
                        ...
                        if ((entriesCount > 1) && (ODlist.MaxSub[i] == 1))
                        { // ODlist.MaxSub[i]>2说明有多个entries //
...
bool EctBus::sendSdoRead(const uint16_t slave, const uint16_t index, const uint8_t subindex, const bool completeAccess, std::vector<uint8_t> &value)
{
...
#ifdef MEASURE_DELAY
    std::chrono::time_point<std::chrono::high_resolution_clock> timeBeign = std::chrono::high_resolution_clock::now();
#endif
    ...
    int wkc = 0;
    {
        ...
        wkc = ecx_SDOread(&ecx_context_ins, slave, index, subindex, static_cast<boolean>(completeAccess), &size, &value.data()[0], EC_TIMEOUTRXM);
        // TDEBUG("wkc={}, size={}, expected_size={}", wkc, size, expected_size);
    ...
    auto duration = (timeEnd - timeBeign);
    std::cout << "time of sendSdoRead:" << double(duration.count()) / 1000000.0 << " ms" << std::endl;
#endif
