...
bool BusBase::busIsAvailable() const { return busIsAvailable(mName); }

int BusBase::getNumberOfSlaves() const {
  std::lock_guard<std::recursive_mutex> guard(contextMutex_);
  return *ecatContext_.slavecount;
}

bool BusBase::addSlave(const SlaveBasePtr& slave) {
...
bool BusBase::startup(const bool sizeCheck) {
  std::lock_guard<std::recursive_mutex> guard(contextMutex_);
  /*
  ...
  if (ecx_init(&ecatContext_, mName.c_str()) <= 0) {
    MELO_ERROR_STREAM("[" << getName() << "] "
  ...
  if (sizeCheck) {
    for (const auto& slave : slaves_) {
      const SlaveBase::PdoInfo pdoInfo = slave->getCurrentPdoInfo();
...
void BusBase::setState(const uint16_t state, const uint16_t slave) {
  std::lock_guard<std::recursive_mutex> guard(contextMutex_);
  if(!initlialized_) {
    MELO_ERROR_STREAM("Bus " << mName << " was not successfully initialized, skipping operation");
    return;
  }
...
bool BusBase::waitForState(const uint16_t state, const uint16_t slave, const unsigned int maxRetries, const double retrySleep) {
  assert(static_cast<int>(slave) <= getNumberOfSlaves());
  std::lock_guard<std::recursive_mutex> guard(contextMutex_);
...
int BusBase::getExpectedWorkingCounter(const uint16_t slave) const {
  assert(static_cast<int>(slave) <= getNumberOfSlaves());
  std::lock_guard<std::recursive_mutex> guard(contextMutex_);
  return ecatContext_.grouplist[slave].outputsWKC * 2 + ecatContext_.grouplist[slave].inputsWKC;
}

std::string BusBase::getErrorString(ec_errort error) {
...
void BusBase::printALStatus(const uint16_t slave) {
  std::lock_guard<std::recursive_mutex> guard(contextMutex_);
  assert(static_cast<int>(slave) <= getNumberOfSlaves());
...
bool BusBase::checkForSdoErrors(const uint16_t slave, const uint16_t index) {
...
bool BusBase::workingCounterIsOk() const { return wkc_ >= getExpectedWorkingCounter(); }

bool BusBase::busIsOk() const { return workingCounterTooLowCounter_ < maxWorkingCounterTooLow_; }

void BusBase::syncDistributedClock0(const uint16_t slave, const bool activate, const double cycleTime, const double cycleShift) {
  MELO_INFO_STREAM("Bus '" << mName << "', slave " << slave << ":  " << (activate ? "Activating" : "Deactivating")
  ...
  MELO_INFO_STREAM("Bus '" << mName << "', slave " << slave << ":  " << (activate ? "Activated" : "Deactivated")
                           << " distributed clock synchronization.");
}

BusBase::PdoSizeMap BusBase::getHardwarePdoSizes() {
  PdoSizeMap pdoMap;
...
BusBase::PdoSizePair BusBase::getHardwarePdoSizes(const uint16_t slave) {
